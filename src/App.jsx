...
function extractCommandeBatiBrico(text){
 ...
 const rebuilt=flat
  .replace(/(\b[A-Z]?\d{5,12}[A-Z0-9]*\s+\(Notre réf:)/g,'\n$1')
  .split(/\n+/)
  .map(clean)
  .filter(l=>l.length>2)

 const lines=[...rawLines,...rebuilt]
 const rows=[]

 for(const line of lines){
  if(isAdminLine(line))continue

  const m=line.match(/^([A-Z]?\d{5,12}[A-Z0-9]*)\s+\(Notre réf:\s*([A-Z0-9]+)\s+(.+)$/i)
  if(!m)continue
  ...
  const qtys=extractQtyUnits(rest)
  if(qtys.length<2)continue

  // V5 : lecture ligne par ligne BATI BRICO
  // premier couple = conditionnement, deuxième couple = quantité commandée réelle
  const qCond=qtys[0]
  const qCmd=qtys[1]
...
export default function App(){
 ...
 return <div className="app">
  <header><div><h1>BATI BRICO</h1><p>Contrôle ARC fournisseur V5 — lecture ligne par ligne BATI BRICO</p></div><div className="status"><span>Contrôle</span><b>{results.length?`${ok} OK / ${verif} à vérifier / ${ec+man+sup} alerte(s)`:'En attente'}</b></div></header>
  ...
  {tab==='import'&&<main className="grid"><section className="card"><h2><Upload/>Commande BATI BRICO</h2><p>Lecture ligne par ligne du format commande BATI BRICO.</p><input type="file" accept=".pdf,.txt,.csv,image/*" onChange={e=>importFile(e.target.files[0],'cmd')}/><textarea value={cmdText} onChange={e=>setCmdText(e.target.value)} placeholder="Texte commande extrait automatiquement ou collé ici..."/><button onClick={parseCmd}><Wand2 size={16}/>Extraire commande BATI</button></section><section className="card"><h2><FileText/>ARC / confirmation fournisseur</h2><p>PDF, scan, image ou confirmation fournisseur.</p><input type="file" accept=".pdf,.txt,.csv,image/*" onChange={e=>importFile(e.target.files[0],'arc')}/><textarea value={arcText} onChange={e=>setArcText(e.target.value)} placeholder="Texte ARC extrait automatiquement ou collé ici..."/><button onClick={parseArc}><Wand2 size={16}/>Extraire ARC</button></section><section className="card wide notice">{loading?<Loader2 className="spin"/>:<ScanText/>}<b>État :</b> {status||'En attente d’import.'}<br/>V5 : commande BATI BRICO lue ligne par ligne, puis comparaison avec ARC universel.<br/><button className="compare" onClick={parseBoth}><RefreshCw size={16}/>Extraire les 2 documents et comparer</button></section></main>}
