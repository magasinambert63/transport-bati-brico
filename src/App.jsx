...
function looksPostalOrAdmin(ref,rest){
 ...
 if(/^(0?7\d{7,}|63\d{3}|86\d{3}|42\d{3}|57\d{3}|67\d{3})$/.test(r) && !articleWords.test(rest))return true
 return false
}
...
function dedupe(rows){
 ...
 for(const r of rows){
  const k=normalizeRef(r.reference)+'|'+(r.quantite||'')+'|'+(r.designation||'').slice(0,26)
...
function extractCommandeBatiBrico(text){
 ...
 const rows=[]
 const re=/(\b[A-Z]?\d{5,12}[A-Z0-9]*\b)\s*\(Notre réf:?\s*([A-Z0-9]+)\)?\s+(.+?)\s+(\d+(?:[,.]\d+)?)\s*([A-Z]{2,4})\s+(\d+(?:[,.]\d+)?)\s*([A-Z]{2,4})\s+(\d+(?:[,.]\d+)?)/gi
 let m
 while((m=re.exec(flat))!==null){
  const ref=m[1]
  const refBati=m[2]
  let designation=clean(m[3])
  if(!designation||isAdminLine(designation)||isEan(ref))continue
  const qCond=fmt(qte(m[4]))
  const uCond=normalizeUnit(m[5])
  const qCmd=fmt(qte(m[6]))
  const uCmd=normalizeUnit(m[7])
  rows.push({
   ...
   quantite:qCmd,
   unite:uCmd,
   conditionnement:`${qCond} ${uCond} / ${qCmd} ${uCmd}`,
   source:clean(m[0]),
...
export default function App(){
 ...
 return <div className="app">
  <header><div><h1>BATI BRICO</h1><p>Contrôle ARC fournisseur V4.1 — commande corrigée</p></div><div className="status"><span>Contrôle</span><b>{results.length?`${ok} OK / ${verif} à vérifier / ${ec+man+sup} alerte(s)`:'En attente'}</b></div></header>
